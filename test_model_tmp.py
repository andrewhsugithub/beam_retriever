...
def load_saved(model, path, exact=True):
    ...
    if exact:
        ...
        state_dict = {
            ...
            if filter(k) in model.state_dict()
        }
...
def get_retr_output(test_raw_data, type="musique", is_dev=True, beam_size=1):
    ...
    if type == "musique":
        ...
        re_checkpoint = "checkpoints/hotpotqa/hotpot_beam1_retr.pt"
    ...
    for sample in tqdm(test_raw_data, desc="RE Predicting:"):
        ...
        if type == "hotpot" or type == "2wiki":
            for idx, (title, sentences) in enumerate(sample["context"]):
                ...
                encoding = tokenizer.encode(
                    ...
                    return_tensors="pt",
                    truncation=True,
                    max_length=max_len - q_codes.shape[-1],
                ).squeeze(0)
        ...
        if type == "2wiki" and sample["type"] == "bridge_comparison":
            hop = 4
        with torch.no_grad():
            current_preds = re_model(
                q_codes_input, c_codes_input, [] if not is_dev else sf_idx, hop=hop
            )["current_preds"]
...
def get_reader_qa_output(
    ...
    qa_model = Reader(
        config,
        qa_model_path,
        len(tokenizer) if ("deberta" not in qa_tokenizer_path) else 0,
    )
    ...
    for sample in tqdm(test_raw_data, desc="QA Predicting:"):
            ...
            for idx, (title, sentences) in enumerate(sample["context"]):
                if idx in sp_list:
                    ...
                    encoding = tokenizer.encode(
                        ...
                        add_special_tokens=False,
                        truncation=True,
                        max_length=max_len - len(q_codes),
                    )
                    c_codes.append(encoding)
        elif type == "musique":
            # musique
        ...
        if type == "hotpot":
            ...
            with torch.no_grad():
                outputs = qa_model(
                    input_ids, attention_mask, sentence_index=SEP_index[0]
                )
        ...
        if is_dev:
            ...
            if type == "hotpot":
                ...
                print(
                    f"sp em:{sum(sp_em_tot) / len(sp_em_tot)}, sp f1:{sum(sp_f1_tot) / len(sp_f1_tot)}"
                )
    if is_dev:
        print(f"em:{sum(em_tot) / len(em_tot)}, f1:{sum(f1_tot) / len(f1_tot)}")
        if type == "hotpot":
    ...
    if type == "musique":
        ...
        with open(pred_filename, "w", encoding="utf-8") as f:
            json.dump(
                {"answer": ans_pred, "sp": sp_pred}, f, ensure_ascii=False, indent=4
            )
...
if __name__ == "__main__":
    ...
    get_reader_qa_output(
        retr_json, test_raw_data, is_dev=is_dev, type=type, answer_merge=True, topk=10
    )
